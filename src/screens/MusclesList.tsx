...
const MuscleList = () => {
  ...
  const handleNavigation = () => {
    navigation.navigate('MuscleWorkoutList');
  };

  return (
    <StyledScrollView className="w-full h-full bg-black">
      {muscleGroups.map((group,index) => (
        <StyledPressable key={index} onPress={handleNavigation} className="bg-violet-600 mt-5 mx-4 p-4 rounded-2xl flex-row items-center justify-between">
